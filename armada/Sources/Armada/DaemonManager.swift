...
final class WallpaperDaemonDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // Set up wake observer to restore wallpapers after sleep
        wakeObserver = NSWorkspace.shared.notificationCenter.addObserver(
            forName: NSWorkspace.didWakeNotification,
            object: nil,
            queue: .main
        ) { [weak self] _ in
            // Delay slightly to allow system to fully wake and reinitialize displays
            DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
                self?.restoreWallpapers()
            }
        }

        // Also observe screen parameter changes (display reconnection after wake)
        screenWakeObserver = NotificationCenter.default.addObserver(
            forName: NSApplication.didChangeScreenParametersNotification,
            object: nil,
            queue: .main
        ) { [weak self] _ in
            // Delay to let screen configuration stabilize
    ...
    private func restoreWallpapers() {
        ...
        for screen in NSScreen.screens {
            ...
            // Check if this display already has the correct wallpaper
            if let existingWindow = DesktopWindowManager.shared.activeWindows[displayID] {
                // Window exists, force resume to handle stuck players after sleep
                existingWindow.forceResume()
                continue
            }

            // No window for this display, restore from settings
